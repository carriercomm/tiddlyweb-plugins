        ...
        def tiddler_dump(self,tiddler,baseurl,active_resource=False,atomconfig={}):
            ...
            if atomconfig:    
                if bagname in atomconfig['bags']:                
                    mappings = atomconfig['bags'][bagname]
                if active_resource:
            ...
            def mapper(atom_field_name,tiddler,default):
                if atom_field_name in mappings:
                  ...
                  if atom_field_name == 'link' and val.startswith("/"):
                    sh =config['server_host']
                    val = u"%s://%s%s"%(sh['scheme'],sh['host'],val)
                    wikified = "http://%s"%wikitext_to_wikklyhtml(baseurl, '', val[7:], self.environ,tiddler=tiddler)
                  else:
                    wikified = wikitext_to_wikklyhtml(baseurl, '', val, self.environ,tiddler=tiddler)
                  return cgi.escape(wikified)
